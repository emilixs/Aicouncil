...
describe('calculateTopicOverlap', () => {
  it('should return 1.0 for identical topic sets', () => {
    expect(calculateTopicOverlap(['api', 'auth'], ['api', 'auth'])).toBeCloseTo(
      1.0,
    );
  });
  ...
  it('should return 0.2 floor for completely disjoint topics', () => {
    expect(
      calculateTopicOverlap(['api', 'auth'], ['database', 'migration']),
    ).toBeCloseTo(0.2);
  });
...
describe('calculateEffectiveRelevance', () => {
  it('should apply decay formula to base relevance', () => {
    const twoWeeksAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000);
    // 1.0 * 0.95^2 = 0.9025
    expect(calculateEffectiveRelevance(1.0, twoWeeksAgo)).toBeCloseTo(
      0.9025,
      2,
    );
  });
  ...
  it('should have a floor of 0.1', () => {
    ...
    expect(
      calculateEffectiveRelevance(1.0, longAgo),
