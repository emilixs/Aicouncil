...
export class CouncilService {
  ...
  /**
   * Start a multi-agent discussion for a session (fire-and-forget).
   * Validates session and experts synchronously, transitions to ACTIVE,
   * then kicks off the discussion loop in the background.
   *
   * @param sessionId - The session ID to start
   * @returns The session in ACTIVE status (loop runs in background)
  ...
  async startDiscussion(sessionId: string): Promise<SessionResponseDto> {
    ...
    this.runDiscussionLoop(sessionId, session, experts).catch((err) =>
      this.logger.error(`Discussion loop error for session ${sessionId}: ${err.message}`, err.stack),
    );
  ...
  /**
   * Runs the discussion loop for a session. Called as fire-and-forget from startDiscussion.
   */
