  ...
  async startDiscussion(sessionId: string): Promise<SessionResponseDto> {
    ...
    try {
      ...
      const memoryPromises = experts
        ...
        .map((expert) => this.memoryService.generateSessionMemory(expert.id, sessionId));
      Promise.allSettled(memoryPromises).then((results) => {
        for (const result of results) {
          if (result.status === 'rejected') {
            this.logger.error(`Failed to generate memory: ${result.reason?.message}`);
          }
        }
      });
